...
class AgroReport(FPDF):
    def header(self):
        ...
        self.cell(0, 10, f'AgroVision AI - Inteligência Geográfica | {datetime.now().strftime("%d/%m/%Y")}', 0, 0, 'C')
        self.ln(20)
...
def create_pdf_report(report_dir: str, client_name: str = "Cliente Especial") -> str:
    ...
    pdf.cell(0, 5, f'Área Analisada: Fazenda Santo Antônio (Processamento Big Data)', ln=True)
    pdf.ln(5)
    ...
    pdf.set_font('Arial', 'B', 12)
    pdf.multi_cell(0, 10, f"ESTIMATIVA DE ECONOMIA: Com base no custo médio de R$ {custo_ha:.2f}/ha, esta operação gerou uma poupança estimada de R$ {poupanca:.2f} para o produtor.", border=1, align='C', fill=True)
