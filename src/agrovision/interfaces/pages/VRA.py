...
def create_shapefile_zip(gdf, output_dir):
    ...
    gdf_shp = gdf.copy()
    if "weed_count" in gdf_shp.columns:
        gdf_shp = gdf_shp.rename(columns={"weed_count": "w_count"})
    if "spray_action" in gdf_shp.columns:
        gdf_shp = gdf_shp.rename(columns={"spray_action": "spray"})
    if "rate_l_ha" in gdf_shp.columns:
        gdf_shp = gdf_shp.rename(columns={"rate_l_ha": "rate"})
...
try:
    ...
    if raw_gdf.empty:
        ...
        m = folium.Map(
            location=[center_lat, center_lon], zoom_start=20, max_zoom=24, tiles="CartoDB positron"
        )
        ...
        if ORTHO_PATH and os.path.exists(ORTHO_PATH):
            ...
            raster_layers.ImageOverlay(
                image=img_data, bounds=bounds, opacity=0.8, name="Ortomosaico HD"
            ).add_to(m)
        ...
        folium.GeoJson(
            ...
            tooltip=folium.GeoJsonTooltip(
                fields=["weed_count", "rate_l_ha"], aliases=["Plantas:", "Taxa (L/ha):"]
            ),
        ).add_to(m)
        ...
        if RAW_PATH and os.path.exists(RAW_PATH):
            ...
            folium.GeoJson(
                ...
                tooltip=folium.GeoJsonTooltip(
                    fields=["class", "confidence"], aliases=["Classe:", "Confiança:"]
                ),
            ).add_to(m)
        ...
        if os.path.exists(report_pdf_path):
            with open(report_pdf_path, "rb") as pdf_file:
                c3.download_button(
                    ...
                    pdf_file,
                    "Relatorio_AgroVision_Cliente.pdf",
                    "application/pdf",
                    type="primary",
                    use_container_width=True,
                )
